...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const mobileMenuLinks = document.querySelectorAll(
    '.mobile-menu .has-children > div > a'
  );
  ...
  // Multi-column submenu functionality - Better distribution with 4 columns
  function initMultiColumnSubmenus() {
    const multiColumnSubmenus = document.querySelectorAll(
      '.multi-column-submenu'
    );

    multiColumnSubmenus.forEach((submenu) => {
      ...
      let columnCount = 1;
      if (itemCount >= 12) {
        columnCount = 4; // For 12+ items, use 4 columns (like 4/4/4/3 for 15 items)
      } else if (itemCount >= 8) {
        columnCount = 3; // For 8-11 items, use 3 columns
      } else if (itemCount >= 6) {
        columnCount = 2; // For 6-7 items, use 2 columns
      }

      // Only restructure if we need multiple columns
      if (columnCount > 1) {
        restructureSubmenu(submenu, items, columnCount);
      }
      ...
      function adjustForScreenSize() {
        if (window.innerWidth <= 768) {
          ...
          if (submenu.querySelector('.submenu-column')) {
            flattenSubmenu(submenu);
          }
          ...
          if (columnCount > 3) {
            restructureSubmenu(submenu, items, 3);
          }
        } else {
          // Large desktop: restore original column count
          if (columnCount > 1) {
            restructureSubmenu(submenu, items, columnCount);
          }
        }
  ...
  // Function to restructure submenu into columns for even distribution
  function restructureSubmenu(submenu, items, columnCount) {
  ...
  // Function to flatten submenu back to single column
  function flattenSubmenu(submenu) {
    const columns = submenu.querySelectorAll('.submenu-column');
    const allItems = [];

    // Collect all items from columns
    columns.forEach((column) => {
      const items = Array.from(column.querySelectorAll('li'));
      allItems.push(...items);
    });

    // Clear submenu and add items back directly
    submenu.innerHTML = '';
  ...
  // Initialize multi-column submenus
  initMultiColumnSubmenus();
  ...
  scrollToTopButton.className =
    'fixed bottom-4 right-4 md:bottom-8 md:right-8 text-white p-3 rounded-full shadow-lg transition-all duration-300 opacity-0 pointer-events-none z-50 hover:scale-110';
            ...
            <div class="${bgColor} border border-gray-200 rounded-lg shadow-lg p-4">
                <div class="flex items-start">
                    ...
                    <div class="ml-4 flex-shrink-0">
                        <button type="button" class="notification-close ${textColor} hover:${textColor
      .replace('text-', 'bg-')
      .replace(
        '-800',
        '-100'
      )} rounded-md inline-flex focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-${bgColor.replace(
      'bg-',
      ''
    )} focus:ring-${textColor.replace('text-', '')}">
  ...
  function handleContactForm(form) {
    ...
    form.addEventListener('submit', function (e) {
      ...
      if (!isValid) {
        const currentLang = getCurrentLanguage();
        showNotification(
          notificationMessages[currentLang].validation,
          'warning'
        );
        return;
      }
